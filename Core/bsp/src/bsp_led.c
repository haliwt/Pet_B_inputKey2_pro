...
void Tape_Led_Filcker(void)
{
   ...
   static uint8_t tape_on_flag,tape_off_flag,tape_on_default= 0xff,tape_off_default=0xff;
    if(led_t.gTimer_flicker < 1){ //500ms

      if(tape_off_default != tape_off_flag){  
          tape_off_default = tape_off_flag ;
          tape_on_flag++;
	     TAPE_LED_OFF();
	  ...
	  if(tape_on_default != tape_on_flag){
          tape_on_default = tape_on_flag;
          tape_off_flag ++;
	     TAPE_LED_ON();
...
void Fan_Led_Flicker(void)
{
   ...
   static uint8_t fan_on_flag,fan_off_flag,fan_on_default= 0xff,fan_off_default=0xff;

    if(led_t.gTimer_flicker < 1){ //500ms

       if(fan_off_default != fan_off_flag){  
          fan_off_default = fan_off_flag ;
          fan_on_flag++;
	
		   FAN_LED_OFF();// FAN_LED_ON()  ;
        }      
	
	   }
	   else if(led_t.gTimer_flicker >0 && led_t.gTimer_flicker <2){
	
         if(fan_on_default != fan_on_flag){
          fan_on_default = fan_on_flag;
          fan_off_flag ++;
...
void Sterilization_Led_Filcker(void)
{
    ...
    static uint8_t ster_on_flag,ster_off_flag,ster_on_default= 0xff,ster_off_default=0xff;
    if(led_t.gTimer_flicker < 1){ //500ms

        if(ster_off_default != ster_off_flag){  
          ster_off_default = ster_off_flag ;
          ster_on_flag++;
          ...
          if(ster_on_default != ster_on_flag){
          ster_on_default = ster_on_flag;
          ster_off_flag ++;
          KILL_LED_OFF();
            }
...
void Keep_Heat_Led_Filcker(void)
{
    ...
    static uint8_t heat_on_flag,heat_off_flag,heat_on_default= 0xff,heat_off_default=0xff;
    if(led_t.gTimer_flicker < 1){ //500ms
	
        if(heat_off_default != heat_off_flag){  
          heat_off_default = heat_off_flag ;
          heat_on_flag++;
            KEEP_HEAT_LED_ON();
		...
		if(heat_on_default != heat_on_flag){
          heat_on_default = heat_on_flag;
          heat_off_flag ++;
	     KEEP_HEAT_LED_OFF();
          }
