		...
		if( xResult == pdPASS )
		{
			...
			if((ulValue & FUN_KEY_0) != 0)
			{
                 ...
                 if(confirm_long_key_flag != 1)
                    pro_t.key_value = confirm_short_key;

                 
                  confirm_long_key_counter=0;
                
            }
            else if((ulValue & FUN_LONG_KEY_2) != 0){
                
                   pro_t.key_value = fun_long_key;
                   fun_long_key_flag = 1;
                   pro_t.gTimer_pro_long_key_timer =0;
                  
            }
            else if((ulValue &  CONFIRM_LONG_KEY_3) != 0){

                 pro_t.key_value  = confirm_long_key;
        ...
        while(FUN_KEY_VALUE()== KEY_DOWN && fun_key_long_counter < 2965500){
               ...
               if(fun_key_long_counter > 2960000){
                ...
                xTaskNotify(xHandleTaskMsgPro, /* 目标任务 */
                            FUN_LONG_KEY_2,            /* 设置目标任务事件标志位bit0  */
