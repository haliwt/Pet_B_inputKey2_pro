...
//  static uint8_t cnt;
 ...
 // uint8_t   	value2 = 0;
	...
	//if(!gpro_t.key_detected_flag)  //10ms check once 
	//	return value1;
  //    gpro_t.key_detected_flag =0;
  ...
  else if( CONFIRM_KEY_VALUE()==KEY_DOWN && gpro_t.long_key_flag ==0 ){
		cnt = 0;
		K2++;   //Confirm_key press
		if(gpro_t.key_as_numbers_input_flag ==1){
           if(K2 > 90000){//K2 > 110000 K2 > 199000
              K2=0;
			  cnt = 0;
		      K1 = 0;
			  value1=0;
  ...
  else if(FUN_KEY_VALUE()==KEY_UP && CONFIRM_KEY_VALUE()==KEY_UP && gpro_t.long_key_flag ==0){ //oneself key 
		cnt++;
