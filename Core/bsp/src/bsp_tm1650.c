...
const uint8_t  segL[1] = {seg_d+seg_e+seg_f}; //display "L"
...
void Smg_Display_Temp_Degree_And_Char_L_Handler(uint8_t temp_value)
{
      uint8_t decade_temp=0,uint_temp=0;//decimal_point;
	...
	TM1650_Write_Data(0x6A,segNumber[uint_temp]);//bit two,don't deciaml 
    

    //display child lock word "L"
    //TM1650_Write_Data(0x6C,segNumber[decimal_point]);//decimal_point
